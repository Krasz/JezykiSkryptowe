...
def game_controls():
    ...
    while gcont:
        ...
        message_to_screen("Klawisze", green, -100, size="large")
        message_to_screen("Strzał: Spacja", black, -30)
        message_to_screen("Ruszanie wieżyczką: góra i dół", black, 10)
        message_to_screen("Poruszanie czołgiem: lewo i prawo", black, 50)
        message_to_screen("Pauza: P", black, 90)

        button("Graj", 150, 500, 100, 50, green, light_green, action = "play")
        button("Menu Główne", 350, 500, 100, 50, yellow, light_yellow, action = "majn")
        button("Wyjście", 550, 500, 100, 50, red, light_red, action = "quit")
...
def pause():
    ...
    paused = True
    message_to_screen("Pauza",black,-100,size="large")
    message_to_screen("Naciśnij C aby kontynuować grę lub Q aby wyjść",black,25)
    pygame.display.update()
    while paused:
        for event in pygame.event.get():
                if event.type == pygame.QUIT:
...
def fireShell(xy, tankx, tanky, turPos, gun_power, xlocation, barrier_width, randomHeight, enemyTankX, enemyTankY):
    ...
    while fire:
        ...
        if startingShell[1] > display_height - ground_height:
            #print("Ostatni pocisk:", startingShell[0], startingShell[1])
            hit_x = int((startingShell[0] * display_height - ground_height)/startingShell[1])
            hit_y = int(display_height - ground_height)
            #print("Kontakt:", hit_x, hit_y)
...
def power(level):
    text = smallfont.render("Siła: " + str(level) + "%", True, black)
...
def game_intro():
    ...
    while intro:
        ...
        gameDisplay.fill(white)
        message_to_screen("Tanki!",green,-100,size="large")
        message_to_screen("Zasady: strzelaj aby zniszczyć",black,-30)
        message_to_screen("przeciny czołg zanim on zniszczy Ciebie.",black,10)

        button("Graj", 150, 500, 100, 50, green, light_green, action = "play")
        button("Sterowanie", 350, 500, 100, 50, yellow, light_yellow, action = "controls")
        button("Wyjście", 550, 500, 100, 50, red, light_red, action = "quit")
...
def game_over():
    ...
    while game_over:
        ...
        gameDisplay.fill(white)
        message_to_screen("Koniec gry",green,-100,size="large")
        message_to_screen("Nie żyjesz",black,-30)

        button("Zagraj ponownie", 150, 500, 150, 50, green, light_green, action = "play")
        button("Sterowanie", 350, 500, 100, 50, yellow, light_yellow, action = "controls")
        button("Wyjście", 550, 500, 100, 50, red, light_red, action = "quit")
...
def you_win():
    ...
    while win:
        ...
        gameDisplay.fill(white)
        message_to_screen("Wygrana",green,-100,size="large")
        message_to_screen("Gratulacje",black,-30)
...
def gameLoop():
    ...
    while not gameExit:
        ...
        if gameOver == True:
            #gameDisplay.fill(white)
            message_to_screen("Koniec Gry", red, -50, size="large")
            message_to_screen("Kliknij C aby zagrać ponownie lub Q aby wyjść z gry", black, 50)
            pygame.display.update()
            while gameOver == True:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
